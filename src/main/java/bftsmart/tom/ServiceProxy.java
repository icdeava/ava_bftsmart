...
//                        request.length;
			...
			TOMulticastReconfig(request, reqId, operationId, reqType);
		}
		...
		try {
			if(reqType == TOMMessageType.UNORDERED_HASHED_REQUEST){
				...
				if (!this.sm.tryAcquire(timeout, TimeUnit.SECONDS)) {
					logger.info("###################TIMEOUT#######################");
					logger.info("invokeWithTimeout: Reply timeout for reqId=" + reqId +
                                                ", Replies received: " + receivedReplies+ ", "+ Arrays.toString(replies));
					canSendLock.unlock();
...
//						ret = response.getContent();
                                                return invokeWithTimeout(request, reqType, 10, req_id);
		...
		try {
			if(reqType == TOMMessageType.UNORDERED_HASHED_REQUEST){
				...
				if (!this.sm.tryAcquire(invokeTimeout, TimeUnit.SECONDS)) {
					logger.info("###################TIMEOUT#######################");
					logger.info("invoke: Reply timeout for reqId=" + reqId + ", Replies received: " + receivedReplies);
					canSendLock.unlock();
...
//					return null;
	...
	public void replyReceived(TOMMessage reply) {
			...
			if (reply.getReqType()== RECONFIG)
			{
				logger.debug("Synchronously received RECONFIG reply from " + reply.getSender() + " with sequence number " + reply.getSequence());
...
//						if(replies[i]!=null && replies[i].getReqType()==TOMMessageType.RECONFIG)
//						{
//							logger.info("replies[i] : {}, seq: {},opid: {}, id: {}, sender: {}, " +
//											"reqtype: {} ", replies[i],replies[i].getSequence(),
//									replies[i].getOperationId(), replies[i].getId(), replies[i].getSender(),replies[i].getReqType() );
//
//						}
                                                        ...
                                                        && replies[i] != null)
						{
							...
							if ( replies[i].getReqType()!= RECONFIG && comparator.compare(replies[i].getContent(),
									reply.getContent()) == 0)
							{
							...
							if (replies[i].getReqType()== RECONFIG)
							{

								if (replies[i].getSequence() == reply.getSequence() && replies[i].getOperationId() == reply.getOperationId() )
								{


									sameContent++;
									logger.debug("reconfig current view process count: "+this.getViewManager().getCurrentViewN());

									if ((sameContent >= this.getViewManager().getCurrentViewN() -1) )
									{
				...
				if (response == null) {
					...
					}else if (requestType.equals(RECONFIG)) {
                                                logger.debug("3, response==null, might be releasing sm "
...
//						+ " with reqId:" + reply.getSequence() + ". Currently wait reqId= " + reqId);
    ...
    public byte[] special_invoke(byte[] request, TOMMessageType reqType) {
