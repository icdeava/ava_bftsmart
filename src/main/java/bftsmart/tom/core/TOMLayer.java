...
public final class TOMLayer extends Thread implements RequestReceiver {
    ...
    private int LastLeaderChangeId = -10000;
...
//        return dt.getLastCID();
    ...
    public void StartLeaderChange(SystemMessage sm) {
        ...
        StandardSMMessage smsg = (StandardSMMessage) sm;


        if (smsg.getRegency()==0)
        {
            int[] tgtArray = controller.getCurrentViewAcceptors();

            SMMessage smsgToSend = new StandardSMMessage(smsg.getSender(),
                    smsg.getCID(), TOMUtil.REMOTE_VIEW_CHANGE, 0, null, null, 1, -1);

            communication.send(tgtArray, smsgToSend);
        }
        else if (smsg.getRegency()==1){
            ...
            logger.info("LastLeaderChangeId: {}, msg_cid:{}", LastLeaderChangeId, smsg.getCID());
            if ( ((SMMessage) sm).getCID() > LastLeaderChangeId+500)
            {
                logger.info("Non REDUNDANT Leader Change, sending request to myself");
                communication.send(myself, new LCMessage(-1, TOMUtil.TRIGGER_LC_LOCALLY, smsg.getCID(), null));
            }

            LastLeaderChangeId = smsg.getCID();
    ...
    public void requestReceivedReconfig(ReconfigMessage sm) throws IOException, ClassNotFoundException{
